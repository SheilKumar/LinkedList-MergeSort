...
void LinkedList<T>::insertOrdered(const T& newData) {
  ...
  if (!head_) {
    ...
    Node* track = head_; 
    while (track->next->data < newData) {
      track = track->next; 
    }
    Node* tracklesser = track;
    Node* trackgreater = track->next; 
    newNode->prev = tracklesser; 
    newNode->next = trackgreater; 
    tracklesser->next = newNode;
    trackgreater->prev = newNode;
  }
...
LinkedList<T> LinkedList<T>::merge(const LinkedList<T>& other) const {
  ...
  // the function.while (merged.size_ != right.size_ + left.size_ )
  LinkedList<T> merged;

  Node* trackleft = left.head_;
  Node* trackright = right.head_; 
  
  Node* trackmerge = merged.head_;
  ...
  if (left.empty()) {
    ...
    while ((trackleft != nullptr) || (trackright != nullptr)) {
      if ((trackright == nullptr) || (trackleft->data < trackright->data))   {
        merged.pushBack(trackleft->data);
        trackleft = trackleft->next;
      } else {
        merged.pushBack(trackright->data);
        trackright = trackright->next;
      }
    }
  }
  // -----------------------------------------------------------
